...
Parquet files are preferred because: 
 ...
 more repitition of the codes and description on the top level; 
 - the file preserves the labelled vector format of the hierarchy, it imports both the codes 
 and descriptions as a `tibble::tibble` format table if the haven package has been loaded before download; 
...
Note the R `nanoparquet::read_parquet` function does not currently support labelled vectors.
